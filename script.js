...
function initializeDealsFunctions() {
    console.log("Initializing Daily Deals functions...");
    
    // Check if Firebase is loaded
    if (typeof firebase === 'undefined' || !firebase.database) {
        console.error("Firebase not loaded yet, waiting...");
        setTimeout(initializeDealsFunctions, 1000);
        return;
    }
    
    console.log("Firebase loaded, registering Daily Deals functions...");
    ...
    window.showDealsScreen = function() {
        ...
        // Wait a bit for screen to render then load deals
        setTimeout(function() {
    ...
    console.log("✅ Daily Deals functions registered successfully");
}

// Wait for page and Firebase to load
document.addEventListener('DOMContentLoaded', function() {
    // Wait 3 seconds for Firebase to initialize
    setTimeout(initializeDealsFunctions, 3000);
    
    // Also try when Firebase auth state changes (if you have auth)
    if (typeof firebase !== 'undefined') {
        firebase.auth().onAuthStateChanged(function(user) {
            if (user) {
                // User logged in, initialize deals
                initializeDealsFunctions();
            }
        });
    }
});

// Modified loadDeals function with better error handling
function loadDeals() {
    ...
    if (!dealsList) {
        ...
        alert("Deals screen not properly loaded. Please try again.");
        return;
    }
    
    dealsList.innerHTML = '<p style="text-align: center; color: #777; padding: 20px;">Loading offers, please wait...</p>';
    
    // Check if Firebase is ready
    if (typeof firebase === 'undefined' || !firebase.database) {
        console.error("Firebase not available");
        dealsList.innerHTML = '<p style="text-align: center; color: red;">Firebase not loaded. Please refresh page.</p>';
        return;
    }
    ...
    try {
        ...
        console.log("Fetching deals from Firebase...");
        
        dealsRef.once('value').then((snapshot) => {
            console.log("Deals data received:", snapshot.exists());
            ...
            if (!snapshot.exists()) {
                dealsList.innerHTML = `
                    <div style="text-align: center; padding: 40px 20px;">
                        <p style="color: #777; font-size: 1.2rem; margin-bottom: 20px;">
                            📭 Abhi koi offers nahi hain
                        </p>
                        <p style="color: #555;">
                            Aap pehla offer daalne wale ban sakte hain!
                        </p>
                    </div>
                `;
                return;
            ...
            snapshot.forEach((child) => {
                ...
                if (deal && deal.title && deal.shopName) {
                    ...
                    // Format date
                    const date = deal.timestamp ? 
                        new Date(deal.timestamp).toLocaleDateString('hi-IN') : 
                        'Recently';
                    ...
                    card.innerHTML = `
                        ...
                        <p style="margin: 5px 0; color: #444;">
                            <strong>📝 Details:</strong> ${deal.description || 'No details provided'}
                        </p>
                        <p style="margin: 5px 0; color: #666;">
                            <strong>📂 Category:</strong> ${deal.category || 'Other'}
                        </p>
            ...
            if (!hasValidDeals) {
                dealsList.innerHTML = `
                    <div style="text-align: center; padding: 30px;">
                        <p style="color: #777;">No valid offers found in database</p>
                    </div>
                `;
            }
            ...
            dealsList.innerHTML = `
                <div style="text-align: center; color: red; padding: 20px;">
                    <p>Error loading offers: ${error.message || 'Unknown error'}</p>
                    <p style="font-size: 0.9rem;">Please check internet connection</p>
                </div>
            `;
        });
...
// Modified registerDeal function
function registerDeal() {
    console.log("registerDeal called...");
    
    // Check Firebase
    if (typeof firebase === 'undefined' || !firebase.database) {
        alert("Firebase not loaded. Please wait and try again.");
        return false;
    }
    ...
    if (dealPhone.length !== 10 || isNaN(dealPhone)) {
    ...
    try {
        ...
        const dealData = {
            ...
            postedBy: localStorage.getItem('user_name') || 'Anonymous',
            userPhone: localStorage.getItem('user_phone') || 'Unknown'
        };
        
        // Show loading
        const originalText = document.querySelector('#dealForm button[type="submit"]')?.textContent;
        ...
        dealsRef.set(dealData).then(() => {
            ...
            // Reset button
            if (submitBtn) {
                submitBtn.textContent = originalText || 'Submit Offer';
                submitBtn.disabled = false;
            }
            
            // Auto-reload deals if on deals screen
            if (document.getElementById('deals-screen')?.classList.contains('active')) {
        ...
        console.error("registerDeal error:", error);
        return false;
    }
